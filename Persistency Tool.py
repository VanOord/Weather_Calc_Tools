...
def find_data_start(file_path):
    with open(file_path) as file:
        for line_num, line in enumerate(file):
            ...
            #print(f"Line {line_num}: {row_data}") #For Troubleshooting purposes
            if len(row_data) >= 2:
                return line_num
    return None

def plot_csv_data(file_path, time, investigation): # Main Function
    ...
    # Getting correct datetime index
    #first_value = str(df[time].iloc[0])  # Can create variable to see format for debugging purposes
    ...
    #print(df[585:613]) #For debugging purposes
    ...
    if num_points <= 3653: #10 years at hourly sampling
        freq = 'AS'
    ...
    # Adding tick marks and labels to x axis on plot
