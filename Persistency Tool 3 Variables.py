...
def calculate_persistency_multi(values, window_size, inputted_limits, values_op): #Calculates combined persistency for up to three variables
    ...
    count = np.sum(threshold_met_windows) # This value gives the actual occurence number of overlapping weather windows (Not used as an output currently)
    persistency = (count / len(values[0])) * 100 # Gives persistency percentage
    
    return persistency, count

def calculate_persistency_non_overlapping_multi(values, window_size, inputted_limits, values_op): #Calculates combined persistency for up to three variables when non-overlapping
    ...
    persistency = count / len(values[0]) * 100 #Persistency percentage, count is the occurences
    return persistency, count
...
def main_calculations(file_path, time, investigation1, investigation2, investigation3):
    ...
    sample_df.set_index(time, inplace=True) #Setting time column as the index for now
    ...
    layout = [
        ...
        [sg.Text("Column 2 (optional): "), sg.Combo(header_names2, key='investigation2'), sg.Text("Limit: "), sg.Input(key='-LIMIT2-', size=(15,1))],
        [sg.Text("Column 3 (optional): "), sg.Combo(header_names3, key='investigation3'), sg.Text("Limit: "), sg.Input(key='-LIMIT3-', size=(15,1))],
    ...
    window = sg.Window("Persistency Tool", layout) #2nd PysimpleGUI window
    ...
    if investigate_range: #this case is for if certain months want to be investigated
        ...
        if overlapping_WW == False: #Only adds this to the non-overlapping case
    ...
    if overlapping_WW == False: #Only adding count values for the non-overlapping case
        ...
        table_data_console["Month"] = table_data_console["Month"].apply(lambda x: x.ljust(14))
        # Construct the custom header
        header = "Month".ljust(13) + "Mean Persistency"
    ...
    output_messages.append(table_string) #Only for GUI output
...
# Apply the light gray fill to the std dev section
    ...
    # Apply the gray fill to the mean monthly occurences row
...
#HEATMAP
# Determine the range for applying the heatmap to the persistency table
if overlapping_WW == False:
    heatmap_start_row = 6
    heatmap_start_col = 2
...
# Determine the range for applying the heatmap for the P values section
if overlapping_WW == False:
